    ...
    elseif #masterList ~= #currentList then
        ...
        for i = 1 , #masterList do
            ...
            for j = 1 , #currentList do
                ...
                if masterList[i][1] == currentList[j][1] then
                    isFound = true;
                end
        ...
        for i = 1 , #currentList do
            ...
            for j = 1 , #masterList do
                ...
                if currentList[i][1] == masterList[j][1] then
                    isFound = true;
                end
...
GRMsync.CheckAddAltSyncChange = function ( finalList , lastStep )
    ...
    for name , alts in pairs ( finalList ) do
        ...
        if player then
            ...
            currentAlts = GRM.GetListOfAlts ( guildData[name] , false , GRM_Alts[GRM_G.guildName] );
                    ...
                    if not lastStep then
                        ...
                        for i = 1 , #currentAlts do
                            ...
                            for j = 1 , #alts do
                                ...
                                if currentAlts[i][1] == alts[j][1] then
                    ...
                    if lastStep then
                        ...
                        for i = 1 , #alts do
                            ...
                            for j = 1 , #currentAlts do
                                ...
                                if alts[i][1] == currentAlts[j][1] then
                            ...
                            if not onList then
                                addToAltsGroup = GRM.AddAlt ( name , alts[i][1] , true , finalList[name].altGroupModified );
                                GRM.SyncBirthdayWithNewAlt ( name , alts[i][1] , addToAltsGroup );
                            end
                        end
                    end
...
GRMsync.CheckAltMainChange = function ( msg , sender )
    ...
    GRM.SetMain ( mainName , false , time() );
...
GRMsync.CheckMainSyncChange = function ( msg )
    ...
    if not abortMainChange then 
        if mainStatus == "true" then
            ...
            GRM.SetMain ( mainName , true , mainChangeTimestamp );
...
GRM.NextSyncStep = function( nextStep )
    local actions = { GRMsync.SendJDPackets , GRMsync.SendPDPackets , GRMsync.SendAddAltPackets , GRMsync.SendMainPackets ,
                     GRMsync.SendCustomNotePackets , GRMsync.SendBDayPackets , GRMsync.SendBANPackets , GRMsync.SendCompletionMsg }
                ...
                if GRM.GetNumAlts ( GRMsyncGlobals.TempRoster[exactIndexes[3][i]].altGroup ) > 0 then
                    ...
                    if #tempMessage + GRMsyncGlobals.sizeModifier < 255 then
                        ...
                        for j = 1 , #alts do
                            if j == 1 and ( #tempMessage4 + GRMsyncGlobals.sizeModifier ) >= 255 then
                                ...
                                -- Add the alt...
                                tempMessage = tempMessage .. "?" .. alts[j][1] .. "?" .. tostring ( alts[j][3] );
                        
                                -- String without the header for carryover.
                                tempMessage2 = tempMessage2 .. "?" .. alts[j][1] .. "?" .. tostring ( alts[j][3] );
                    ...
                    if #tempMessage + GRMsyncGlobals.sizeModifier < 255 then
                        ...
                        if #tempMessage + GRMsyncGlobals.sizeModifier < 255 then
                            ...
                            messageReady = true;
                            -- Hold this value over...
                            tempMessage = GRM_G.PatchDayString .. "?GRM_ALTADDSYNC?" .. syncRankFilter .. "?&" .. GRMsyncGlobals.TempRoster[exactIndexes[3][i]].name .. "?" .. GRMsyncGlobals.TempRoster[exactIndexes[3][i]].altGroupModified;
                        end
                    end
                end
                ...
                if GRMsyncGlobals.SyncOK then
                    ...
                    if #tempMessage + GRMsyncGlobals.sizeModifier < 255 then
                        ...
                        if msgTag == "ban" or msgTag == "unban" then
                            ...
                            tempMessage3 = "?#" .. leftGuildData[i].name .. "?" .. tostring ( leftGuildData[i].rankIndex ) .. "?" .. tostring ( leftGuildData[i].level ) .. "?" .. class .. "?" .. tostring ( leftGuildData[i].joinDateHist[4] ) .. "?" .. tostring ( oldJoinDateMeta ) .. "?" .. guid .. "?&" .. leftGuildData[i].name .. "?" .. timeStampOfBanChange .. "?" .. msgTag .. "?" .. reason .. "?" .. playerWhoBanned;
                    ...
                    if GRMsyncGlobals.SyncOK then          
                        ...
                        if #tempMessage + GRMsyncGlobals.sizeModifier < 255 then
                            if guildData[exactIndexes[7][i]].events[2][4] ~= 0 then
                                ...
                                alts = GRM.GetListOfAlts ( guildData[exactIndexes[7][i]] , false , GRMsyncGlobals.TempAltRoster );
                                
                                list = GRMsync.RemoveAltGroupingFromList ( guildData[exactIndexes[7][i]].name , alts , list );
                            end
...
GRMsync.SubmitFinalSyncData = function()
    ...
    if not GRMsyncGlobals.finalSyncProgress[1] and #GRMsyncGlobals.JDChanges > 0 then
        ...
        for i = GRMsyncGlobals.finalSyncDataCount , #GRMsyncGlobals.JDChanges do
            ...
            if GRMsyncGlobals.SyncOK then
                ...
                msg = GRMsyncGlobals.JDChanges[i][1] .. "?" .. tostring ( GRMsyncGlobals.JDChanges[i][5] ) .. "?" .. tostring ( GRMsyncGlobals.JDChanges[i][2] ) .. "?" .. tostring ( GRMsyncGlobals.JDChanges[i][3] ) .. "?" .. tostring ( GRMsyncGlobals.JDChanges[i][4] ) .. "?" .. tostring ( GRMsyncGlobals.JDChanges[i][6] ) .. "?0";
    ...
    if not GRMsyncGlobals.finalSyncProgress[2] and #GRMsyncGlobals.PDChanges > 0 then
        for i = GRMsyncGlobals.finalSyncDataCount , #GRMsyncGlobals.PDChanges do
            ...
            if GRMsyncGlobals.SyncOK then
                ...
                msg = GRMsyncGlobals.PDChanges[i][1] .. "?" .. tostring ( GRMsyncGlobals.PDChanges[i][2] ) .. "?" .. tostring ( GRMsyncGlobals.PDChanges[i][3] ) .. "?" .. tostring ( GRMsyncGlobals.PDChanges[i][4] ) .. "?" .. tostring ( GRMsyncGlobals.PDChanges[i][6] ) .. "?" .. tostring ( GRMsyncGlobals.PDChanges[i][5] );
    ...
    -- BAN changes sync!
    if not GRMsyncGlobals.finalSyncProgress[3] and #GRMsyncGlobals.BanChanges > 0 then
        local playerWhoBanned = "";
    ...
    if not GRMsyncGlobals.finalSyncProgress[4] and GRM.TableLength ( GRMsyncGlobals.FinalCorrectAltList ) > 0 then

        local syncName = ""
        local syncRank = 0;
        local altGroupModified = 0;

        for name in pairs ( GRMsyncGlobals.FinalCorrectAltList ) do

            if GRMsyncGlobals.FinalCorrectAltList[name].finished == nil then

                if GRMsyncGlobals.SyncOK then

                    syncName = GRMsyncGlobals.FinalCorrectAltList[name].syncControl[1];
                    syncRank = GRMsyncGlobals.FinalCorrectAltList[name].syncControl[2];
                    altGroupModified = GRMsyncGlobals.FinalCorrectAltList[name].altGroupModified;

                    -- Sync controls
                    tempMsg1 = GRM_G.PatchDayString .. "?GRM_ALTSYNCUP?" .. syncName .. "?" .. tostring ( syncRank ) .. "?";

                    if #GRMsyncGlobals.FinalCorrectAltList[name] > 0 then

                        for i = GRMsyncGlobals.SyncCountAdd , #GRMsyncGlobals.FinalCorrectAltList[name] do

                            msg = name .. "?" .. GRMsyncGlobals.FinalCorrectAltList[name][i][1] .. "?" .. tostring ( altGroupModified );

                            GRMsyncGlobals.SyncCount = GRMsyncGlobals.SyncCount + #tempMsg1 + #msg + GRMsyncGlobals.sizeModifier;
                            GRMsyncGlobals.SyncCountAdd = GRMsyncGlobals.SyncCountAdd + 1;
                            ...
                            -- Reset the loop at the end
                            if i == #GRMsyncGlobals.FinalCorrectAltList[name] then
                                GRMsyncGlobals.SyncCountAdd = 1;
                            end
                        end
                        ...
                        msg = name .. "?###?" .. tostring ( altGroupModified );
            ...
            GRMsyncGlobals.FinalCorrectAltList[name].finished = true;
        end
        ...
        GRMsync.SendMessage ( "GRM_SYNC" , GRM_G.PatchDayString .. "?GRM_FINALALTSYNCUP?" .. GRMsyncGlobals.DesignatedLeader .. "?" .. tostring ( syncRankFilter ) .. "?" , GRMsyncGlobals.channelName );
    ...
    -- MAIN STATUS CHECK!
    if not GRMsyncGlobals.finalSyncProgress[5] and #GRMsyncGlobals.AltMainChanges > 0 then
    ...
    -- CUSTOM NOTE CHECK!
    if not GRMsyncGlobals.finalSyncProgress[6] and #GRMsyncGlobals.CustomNoteChanges > 0 then
...
GRMsync.CollectData = function ( msg , prefix )
    ...
    if prefix == "GRM_JDSYNC" then
        while string.find ( msg , "?" ) ~= nil do
            ...
            table.insert ( GRMsyncGlobals.JDReceivedTemp , { name , date[1] , date[2] , date[3] , timeStampOfChange , dateInEpoch , GRMsyncGlobals.CurrentSyncPlayer , GRMsyncGlobals.CurrentSyncPlayerRankRequirement } );
        end
    ...
    elseif prefix == "GRM_PDSYNC" then
        while string.find ( msg , "?" ) ~= nil do
            ...
            table.insert ( GRMsyncGlobals.PDReceivedTemp , { name , date[1] , date[2] , date[3] , timeStampOfChange , dateInEpoch , GRMsyncGlobals.CurrentSyncPlayer , GRMsyncGlobals.CurrentSyncPlayerRankRequirement} );
        end
...
GRMsync.CollectAltAddData = function ( msg )
    ...
    -- this is for an edge case... to prevent getting d/c'd
    ...
    while string.find ( msg , "&" ) ~= nil and string.find ( msg , "?" ) ~= nil do -- if only the leader name is sent, but not the alt, we need to break here
        ...
        if tonumber ( string.sub ( msg , 1 , 1 ) ) == nil then      
            ...
            while string.find ( msg , "?" ) ~= nil and string.sub ( msg , 1 , 1 ) ~= "&" do
                ...
                table.insert ( GRMsyncGlobals.AltReceivedTemp[name] , { altName , timestampResult } );
            end
        else
            ...
            if string.find ( msg , "&" ) ~= nil then
                ...
                GRMsyncGlobals.AltReceivedTemp[name].altGroupModified = tonumber ( string.sub ( msg , 1 , string.find ( msg , "?" ) - 1 ) );
                msg = GRM.Next ( msg );
            else
                GRMsyncGlobals.AltReceivedTemp[name].altGroupModified = tonumber ( msg );
            end
        end
    end
end
            ...
            for i = #list , 1 , -1 do
                if guildData[exactIndexes[7][j]].name == list[i][1] then
                    if guildData[exactIndexes[7][j]].events[2][4] ~= 0 then       -- Birthday set!
                        ...
                        alts = GRM.GetListOfAlts ( guildData[exactIndexes[7][j]] , false , GRMsyncGlobals.TempAltRoster );

                        list = GRMsync.RemoveAltGroupingFromList ( guildData[exactIndexes[7][j]].name , alts , list );
                        ...
                        if not isFound or needToAddMyData then
                            ...
                            for k = 1 , #alts do
                                ...
                                for m = 1 , #GRMsyncGlobals.BirthdayReceivedTemp do
                                    if alts[k][1] == GRMsyncGlobals.BirthdayReceivedTemp[m][1] then
    ...
    for j = 1 , #exactIndexes[3] do
        ...
        if #alts > 0 then
            for i = 1 , #alts do
                ...
                table.insert ( leaderListOfAlts[guildData[exactIndexes[3][j]].name] , { alts[i][1] , alts[i][3] } ); -- AN extra step, but easier to follow in the code.
            end
        else
        ...
        if GRMsyncGlobals.AltReceivedTemp[name] ~= nil then  -- If it did == nil, it would indicate a failure in the sync process
            ...
            if #GRMsyncGlobals.AltReceivedTemp[name] > 0 then
                ...
                if #leaderAlts > 0 then
                    ...
                    if GRMsyncGlobals.AltReceivedTemp[name][1][2] > leaderAlts[1][2] then
                        ...
                        for i = 1 , #GRMsyncGlobals.AltReceivedTemp[name] do
                            ...
                            if i == 1 then
                                GRMsyncGlobals.FinalCorrectAltList[name].altGroupModified = GRMsyncGlobals.AltReceivedTemp[name][i][2];
                            ...
                            table.insert ( GRMsyncGlobals.FinalCorrectAltList[name] , { GRMsyncGlobals.AltReceivedTemp[name][i][1] } );
                        ...
                        for i = 1 , #leaderAlts do
                            ...
                            if i == 1 then
                                GRMsyncGlobals.FinalCorrectAltList[name].altGroupModified = leaderAlts[i][2];
                                GRMsyncGlobals.FinalCorrectAltList[name].syncControl = { GRMsyncGlobals.DesignatedLeader , syncRankFilter };
                            end

                            table.insert ( GRMsyncGlobals.FinalCorrectAltList[name] , { leaderAlts[i][1] } );
                    ...
                    if GRMsyncGlobals.AltReceivedTemp[name][1][2] > leaderListOfAlts[name].altGroupModified then
...
GRMsync.PreCheckChanges = function ( msg , banSyncCheck )
    local lists = { "JD" , "PD" , "BAN" , "MAIN" , "ALTS" ,  "CUSTOM" , "BDAY" };
...
local commsSyncUp = { ["GRM_JDSYNCUP"] = true , ["GRM_PDSYNCUP"] = true , ["GRM_ALTSYNCUP"] = true , ["GRM_REMSYNCUP"] = true , ["GRM_MAINSYNCUP"] = true , ["GRM_BDSYNCUP"] = true , ["GRM_BANSYNCUP"] = true , ["GRM_FINALALTSYNCUP"] = true };
